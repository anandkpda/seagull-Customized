...
// Anand added : read IMS-AKA values from config file
#define XML_DIGEST_DIGEST_AKA    (char*)"digest-aka"
#define XML_DIGEST_KEY           (char*)"key"
#define XML_DIGEST_OP            (char*)"op"
#define XML_DIGEST_AMF           (char*)"amf"
#define XML_DIGEST_SQN           (char*)"sqn"
...
bool C_ChannelControl::fromXml (C_XmlData *P_data, 
  ...
  T_pDigestAKA                  L_digest_aka_info = NULL ;
		  ...
		  if (strcmp(L_entity_value, XML_CHANNEL_ENTITY_VALUE) == 0) {
		    ...
		    m_name_map
		      ->insert(T_TransportNameMap::value_type(L_name,
                  ...
                  // Anand Adding start:
                  else if (strcmp(L_entity_value, XML_DIGEST_DIGEST_AKA) == 0) {
                    GEN_DEBUG(1, "C_ChannelControl::fromXml() Anand start");
                    L_key = L_subSection->find_value(XML_DIGEST_KEY); 
		    if (L_key == NULL) {
		      GEN_ERROR(E_GEN_FATAL_ERROR, "[" 
				<< L_key << "] value is mandatory for section ["
				<< XML_CHANNEL_SECTION << "], sub section ["
				<< XML_DIGEST_DIGEST_AKA << "]");
		      L_ret = false ;
		      break ;
		    }

                    L_op = L_subSection->find_value(XML_DIGEST_OP);
                    if (L_op == NULL) {
                      GEN_ERROR(E_GEN_FATAL_ERROR, "["
                                << L_op << "] value is mandatory for section ["
                                << XML_CHANNEL_SECTION << "], sub section ["
                                << XML_DIGEST_DIGEST_AKA << "]");
                      L_ret = false ;
                      break ;
                    ...
                    L_amf = L_subSection->find_value(XML_DIGEST_AMF);
                    if (L_amf == NULL) {
                      GEN_ERROR(E_GEN_FATAL_ERROR, "["
                                << L_amf << "] value is mandatory for section ["
                                << XML_CHANNEL_SECTION << "], sub section ["
                                << XML_DIGEST_DIGEST_AKA << "]");
                      L_ret = false ;
                      break ;
                    ...
                    L_sqn = L_subSection->find_value(XML_DIGEST_SQN);
                    if (L_sqn == NULL) {
                      GEN_ERROR(E_GEN_FATAL_ERROR, "["
                                << L_sqn << "] value is mandatory for section ["
                                << XML_CHANNEL_SECTION << "], sub section ["
                                << XML_DIGEST_DIGEST_AKA << "]");
                      L_ret = false ;
                      break ;
                    }
                    ALLOC_VAR(L_digest_aka_info,
                              T_pDigestAKA,
                              sizeof(T_DigestAKA));

                    L_digest_aka_info->m_key = L_key ;
                    L_digest_aka_info->m_op = L_op ;
                    L_digest_aka_info->m_amf = L_amf ;
                    L_digest_aka_info->m_sqn = L_sqn ;
